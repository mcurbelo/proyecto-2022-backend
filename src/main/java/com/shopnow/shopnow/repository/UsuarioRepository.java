...
public interface UsuarioRepository extends JpaRepository<Usuario, UUID> {
    ...
    @Query(value = "select cast(id as varchar) from usuario where nombre ilike %?1% and estado != 'Eliminado'", nativeQuery = true)
    List<UUID> usuariosConNombre(String nombre);

//    @Query(value = "select cast(id as varchar) from usuario where nombre ilike %?1%" and estado != 'Eliminado', nativeQuery = true)
//    List<UUID> usuariosConNombre(String nombre);

    @Query(value = "select cast(id as varchar) from usuario where apellido ilike %?1% and estado != 'Eliminado'", nativeQuery = true)
    List<UUID> usuariosConApellido(String nombre);

    @Query(value = "select cast(id as varchar) from usuario where correo ilike %?1% and estado != 'Eliminado'", nativeQuery = true)
    ...
    @Query(value = "SELECT cast(id as varchar) FROM (Select lower(concat(nombre, apellido)) as nom, * FROM usuario) as nombreconcat where nom ilike %?1%", nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "Update USUARIO set web_token=NULL where web_token=?1", nativeQuery = true)
    void quitarTokenWeb(String token);

    @Modifying
    @Transactional
    @Query(value = "Update USUARIO set mobile_token=NULL where mobile_token=?1", nativeQuery = true)
