...
public interface ReclamoRepository extends JpaRepository<Reclamo, UUID> {
    ...
    @Query(value = "Select cast (r.id as varchar) from (((usuario_reclamos u join reclamo r on reclamos_key=id) join (compra c join compra_producto cp on c.entrega_info=cp.id) on r.compra_id=c.id)) join producto p on p.id=cp.producto_id where u.generico_id=?1 and p.nombre ilike %?2%", nativeQuery = true)
    List<UUID> misReclamosHechosPorNombreProducto(UUID id, String nombre);

    @Query(value = "Select cast (r.id as varchar) from ((usuario_ventas uv join usuario u on uv.generico_id=u.id) join (usuario_reclamos ur join reclamo r on reclamos_key=id) on r.compra_id=ventas_key) join datos_vendedor dv on u.datos_vendedor_id= dv.id where ur.generico_id=?1 and coalesce(dv.nombre_empresa,u.nombre) ilike %?2% ", nativeQuery = true)
    ...
    @Query(value = "select cast (id as varchar) from reclamo where compra_id in (select c.id from ((usuario_ventas uv join compra c on uv.ventas_key=c.id) join compra_producto cp on c.entrega_info=cp.id) join producto p on cp.producto_id=p.id where uv.generico_id=?1 and p.nombre ilike %?2%)", nativeQuery = true)
    List<UUID> reclamosRecibosPorNombreProducto(UUID id, String nombre);

    @Query(value = "select cast (r.id as varchar) from reclamo r join (usuario usu join usuario_reclamos ur on usu.id=ur.generico_id) on r.id=ur.reclamos_key where usu.nombre ilike %?2% and compra_id in (select ventas_key from usuario_ventas where generico_id=?1)", nativeQuery = true)
